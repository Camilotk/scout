...
class RegistrationManager(models.Manager):
    ...
    def get_or_create_active_user(self, user):
        """
        Metodo para criar novos usuarios com o Registration
        """
        profile_tuple = RegistrationProfile.objects.get_or_create(user=user)
...
class RegistrationProfile(models.Model):
    ...
    user = models.ForeignKey(UserModelString(), unique=True, verbose_name=_('user'), null=False)
    activation_key = models.CharField(_('Chave de Ativação'), max_length=40)
    scout_group = models.ForeignKey(verbose_name=_(u"Grupo Escoteiro"), to=ScoutGroup, null=True, blank=True)
...
def create_registration_profile(sender, instance, **kwargs):
    ...
    if not instance.registrationprofile_set.count():
        profile = RegistrationProfile.objects.get_or_create_active_user(instance)
